...
function markdownize(node) {
	...
	var decoded = (new DOMParser)
		.parseFromString('<!doctype html><body>' + content, 'text/html')
		.body.textContent;
...
function postProcess(state, delimiters) {
	...
	for (i = max - 1; i >= 0; i--) {
		...
		token.type = isStrong ? isUnder ? 'under_open' : 'strong_open' : isUnder ? "em_open" : 'mark_open';
		...
		token.type = isStrong ? isUnder ? 'under_close' : 'strong_close' : isUnder ? "em_close" : 'mark_close';
		token.tag = isStrong ? isUnder ? 'u' : 'strong' : isUnder ? "em" : 'mark';
		token.nesting = -1;
		token.markup = isStrong ? ch + ch : ch;
		token.content = '';
